...
import { appExampleCode, babelConfig, exampleTsConfig, metroConfig } from "./code-snippets/code.js";
import { ANDROID_CXX_LIB_NAME_TAG, ANDROID_NAME_SPACE_TAG, CXX_NAME_SPACE_TAG, IOS_MODULE_NAME_TAG, JS_PACKAGE_NAME_TAG, messages } from "./constants";
...
import { copyTemplateFiles, createFolder, createModuleFile, generateAutolinking, getGitUserInfo, replaceHyphen, replacePlaceholder, toPascalCase } from "./utils";
...
export class NitroModuleFactory {
    ...
    constructor(private config: GenerateModuleConfig) {
        ...
        this.generators = new Map<SupportedLang, FileGenerator>([
            ...
            [SupportedLang.CPP, new CppFileGenerator([androidGenerator, iosGenerator])],
    ...
    private async replaceNitroJsonPlaceholders() {
        const nitroJsonContent = await readFile(path.join(this.config.cwd, 'nitro.json'), { encoding: 'utf-8' });
        const replacements = {
    ...
    private async updatePackageJsonConfig(skipExample?: boolean) {
        ...
        const workspacePackageJsonFilePath = path.join(this.config.cwd, 'package.json')
        const workspacePackageJsonFile = await readFile(workspacePackageJsonFilePath, { encoding: 'utf8' })
    ...
    private async copyPackageFiles() {
        const filesToCopy = ['.watchmanconfig', 'babel.config.js', 'nitro.json', 'tsconfig.json', '.gitignore', 'README.md', 'package.json']
        await copyTemplateFiles(this.config, [__dirname, '..', 'assets', 'template'], filesToCopy);
    }
    ...
    private async createExampleApp() {
        ...
        const reactNativeConfigPath = path.join(this.config.cwd, 'example', 'react-native.config.js')
        ...
        await writeFile(
            appPath,
            appExampleCode(this.config.moduleName, this.config.finalModuleName, `${this.config.funcName}`),
            { encoding: 'utf8' }
        )

        // Setup metro.config.js
        const metroConfigPath = path.join(this.config.cwd, 'example', 'metro.config.js')
        ...
        const babelConfigPath = path.join(this.config.cwd, 'example', 'babel.config.js')
        ...
        const androidSettingsGradlePath = path.join(this.config.cwd, 'example', 'android', 'settings.gradle')
