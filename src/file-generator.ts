...
export class FileGenerator {
    ...
    public async generate({
        ...
        this.finalModuleName =
            `${this.packagePrefix}${moduleName}`.toLowerCase()
    ...
    private async cloneNitroTemplate() {
        ...
        if (exists) {
            ...
            const shouldClone =
                fileStats.mtimeMs < Date.now() - 1000 * 60 * 60 * 24
            if (shouldClone) {
    ...
    private async generateAndroidFiles() {
        ...
        await this.generateModuleFile(
            `android/src/main/java/${this.androidPackageName.split('.').join('/')}/Hybrid${toPascalCase(this.moduleName)}.kt`,
            getKotlinCode(
                this.moduleName,
    ...
    private async generateGradleFile() {
        ...
        const replacements = {
            [`com.margelo.nitro.${ANDROID_NAME_SPACE_TAG}`]:
                this.androidPackageName,
            [ANDROID_CXX_LIB_NAME_TAG]: toPascalCase(this.moduleName),
        }
    ...
    private async generatePackageFile() {
        ...
        await this.generateModuleFile(
            `${prefixPath}/${this.androidPackageName.split('.').join('/')}/${androidPackageFile}`,
            await this.replacePlaceholder({
...
export interface ${toPascalCase(this.moduleName)} extends HybridObject<{ ${platformLang} }> { }
  `
